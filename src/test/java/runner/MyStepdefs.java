...
public class MyStepdefs {
    ...
    public void iSendGETRequestApiAuthenticationTokenJsonWithJson(String method,String url, String jsonBody) {
    ...
    //enviar el token en el metodo i sendPOSTRequestApi
    @When("^I send (POST|PUT|DELETE|GET) request '(.*)' with (.*) and with json$")
    public void iSendPOSTRequestApiItemsJsonWithTOKENAndWithJson(String method,String url,String Token, String jsonBody) {
        RequestInformation request = new RequestInformation();
        request.setUrl(HOST+this.replaceVariables(url));
        request.setBody(this.replaceVariables(jsonBody));
        request.addHeaders(TOKEN_AUTHENTICATION_HEADER,this.replaceVariables(Token));
